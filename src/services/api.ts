...
export const couponApi = {
  ...
  getPaginated: async (page: number = 0, size: number = 10): Promise<PaginatedResponse<Coupon>> => {
    try {
      console.log(`🔍 Making coupon API call to: /api/v1/coupons/paged?page=${page}&size=${size}`);
      const response = await getApiClient().get<any>(
        `/api/v1/coupons/paged?page=${page}&size=${size}`
      );
      console.log("🔍 Raw coupon API response:", response);
      console.log("🔍 Raw coupon API response data:", response.data);
